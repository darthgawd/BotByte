...
if (!fs.existsSync(lcovPath)) {
  console.error('lcov.info not found. Run "forge coverage --report lcov" in contracts/ first.');
  process.exit(1);
}
...
console.log('-----------------------------------------');
fileStats.sort((a, b) => a.file.localeCompare(b.file)).forEach(s => {
    console.log(`${s.file.padEnd(30)} | ${s.coverage}`);
});
console.log('-----------------------------------------');
const totalPercent = totalLines > 0 ? ((totalHits / totalLines) * 100).toFixed(2) : '0.00';
